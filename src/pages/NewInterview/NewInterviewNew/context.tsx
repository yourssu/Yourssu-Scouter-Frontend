...
interface ScheduleCreationContextActions {
  addDraftSchedule: (schedule: DraftSchedule) => void;
  ...
  setDraftSchedules: (schedules: DraftSchedule[]) => void;
...
export const ScheduleCreationProvider = ({ children }: PropsWithChildren) => {
  const { data: semester } = useSuspenseQuery(semesterNowOptions());
  ...
  const addDraftSchedule = useCallback((schedule: DraftSchedule) => {
    setState((prev) => {
  ...
  const setDraftSchedules = useCallback((schedules: DraftSchedule[]) => {
    setState((prev) => ({ ...prev, draftSchedules: schedules }));
  }, []);
