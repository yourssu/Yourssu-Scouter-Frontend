...
import { MailEditDialog } from '@/pages/SendMail/MailEditDialog';
...
export const MailTab = ({ onCompose, statuses }: MailTabProps) => {
    ...
    <FormProvider {...methods}>
      ...
      <div className="flex flex-col gap-3">
        {groupedMails.map((group) => (
          <TemplateList
            date={formatTemplates['2025.01.01'](group[0].reservationTime)}
            key={group[0].mailSubject}
            onClick={() => console.log('메일 상세보기')}
      ...
      {selectedMailIds && (
        <MailEditDialog isOpen mailIds={selectedMailIds} onClose={() => setSelectedMailIds(null)} />
      )}
    </FormProvider>
  );
};
